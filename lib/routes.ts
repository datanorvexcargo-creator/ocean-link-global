...
export const CITIES: City[] = [
  ...
  { name: 'Istanbul',   region: 'Bosphorus · Eurasia', code: 'IST', lat: 41.0082, lng: 28.9784 },
];

// indices into CITIES — continuous global loop
// Istanbul sits between Marseille and Dubai on the Mediterranean → Persian
// Gulf corridor, so the loop now flows through the Bosphorus on its way east.
export const ROUTES: Array<[number, number]> = [
  [0, 6], // Tokyo      -> Long Beach
  [6, 5], // Long Beach -> NYC
  [5, 4], // NYC        -> Rotterdam
  [4, 3], // Rotterdam  -> Marseille
  [3, 7], // Marseille  -> Istanbul   (new)
  [7, 2], // Istanbul   -> Dubai      (new)
  [2, 1], // Dubai      -> Singapore
  [1, 0], // Singapore  -> Tokyo
];
